...
export class SeguroComponent implements OnInit {
  ...
  trackId(index: number, item: ISeguro): number {
    return item.id!;
  }

  delete(seguro: ISeguro): void {
    const modalRef = this.modalService.open(SeguroDeleteDialogComponent, { size: 'lg', backdrop: 'static' });
    modalRef.componentInstance.seguro = seguro;
  ...
  protected onSuccess(data: ISeguro[] | null, headers: HttpHeaders, page: number, navigate: boolean): void {
